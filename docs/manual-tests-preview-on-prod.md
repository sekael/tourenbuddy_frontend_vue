...
| 3 | Email — tour interest | A and B are friends; A has tour T (visibility=`friends`, tour\_type set)                     | B saves a tour within 200 m of T's goal with the same `tour_type`                                       | A receives `tour_interest` email referencing both tours. Verify wording is the new "consider linking" copy, not the legacy "declined duplicate."                                                  |
