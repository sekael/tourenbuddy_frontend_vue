...
- Asset + tile caching, plus an **offline data cache** (change: offline-app-cache-sync): the store-loaded collections (tours, contacts, profile, calendar availability, friendships) are cached in IndexedDB (`core/offline/`) and hydrate offline via a `cachedLoad` hydrate-then-refetch path; a module-level `isOnline` signal gates the network and drives the offline UI.
- Offline data **write sync** (change: offline-write-sync): mutations go through the single `mutate()` seam — online they run directly; offline they are enqueued to a durable IndexedDB **outbox** (`core/offline/write-queue.ts`), applied optimistically, and cache-write-through in ONE transaction (DC2). One coalesced entry per entity; replayed FIFO by `seq` on reconnect via a `kind→handler` registry (`replay.ts`). The reconnect refetch waits for the drain first (`reconnect.ts` `flushThenRefetch`, DC4) so a replayed write is never clobbered by the server snapshot; drains are kickstarted by the realtime `SUBSCRIBED` event + foreground/online nudges (`flush-triggers.ts`). Conflicts are last-write-wins on server `updated_at` vs the entry's `baseUpdatedAt` (DC5); notifications + link-eviction are deferred to the shared success seam so they fire once, online or replayed (DC6). A GPX picked offline is staged in the blob cache (rendering offline via the same `loadCachedBlob` path) and rides the queue entry's `blobs` (marked `pending-upload:<key>`), uploaded to Storage on replay before the row write; tour attachments stay online-only (DC10). Tours + contacts stores are fully wired; profile/availability/friendships wire the same pattern. Contacts are an aggregate (contact + methods): every field/method action builds the whole desired `Contact` and replays as ONE idempotent `create_contact_full`/`update_contact_full` call that reconciles the method set (client-minted ids for contact AND methods) — no multi-write replay.
