...
**Goals:**
- Sheet top stays anchored; sheet shrinks from the bottom to rest above the keyboard; content scrolls.
...
**3. Height via a reactive `restingHeight` base + small-device fallback.**
Introduce a `restingHeight` ref (`S`) that the snap/fit logic writes instead of writing `currentHeight` directly (snap → `snapHeightPx(lastSnap)`, fit → measured fit height). `currentHeight` becomes a function of `(restingHeight, inset, H)`, re-applied by a `watch`:
- `K = 0` → `S`
- `0 < K ≤ S` → `S − K` (top anchored; map keeps its region)
- `K > S` → `H − K` (top reaches 0; sheet takes all space above the keyboard; map sacrificed — acceptable on small devices)
- Rationale: a reactive base avoids the "snapshot captured *when*?" hazard (a snapshot of `currentHeight` mid-transition/mid-drag saves a garbage `S`); restore-on-close is free because the branch yields `S` with no stored state to go stale.
- Alternative considered: imperative snapshot on `inset: 0 → >0`. Rejected for the timing fragility above.

**4. Gate the drag handle while the keyboard is open.**
While `inset > 0`, the drag handle is inert — `onDragStart` early-returns, exactly mirroring the existing `collapsed` guard (extend it to `collapsed || inset > 0`). While typing, the sheet size is keyboard-driven; manual resize is meaningless and the `K > S` branch could flip mid-gesture. `restingHeight` is untouched, so closing the keyboard restores the prior snap.
- Alternatives considered: drag-to-dismiss-keyboard (adds an imperative `blur()` and a surprise) and live-`restingHeight` drag (invites the small-device flip mid-gesture). Both rejected.
...
- **`map.resize()` timing** → the map already resizes on `visualViewport.resize`; the sheet shrink and map canvas resize are driven by the same event, so they stay consistent.
