...
- [x] 2.10 Shared SQL function `fn_collision_predicate(tour_a_id uuid, tour_b_id uuid) RETURNS bool` — single source of truth for the collision predicate (PostGIS `ST_DWithin` ≤ 100 m, both non-null matching `tour_type`, both `visibility = 'friends'`, friendship row exists either direction, ids distinct)
...
- [x] 2.16 Trigger function `fn_evict_member_on_tour_change()`: AFTER UPDATE on `tours` — if `tour_type` changed, evict; if `visibility` changed away from `friends`, evict; if `goal` changed and the new goal is >100 m from any sibling member's goal, evict
...
- [x] 10.1 Collision predicate unit tests: null tour_type, exactly 100 m boundary, different tour_type, one side private, non-friend owner
