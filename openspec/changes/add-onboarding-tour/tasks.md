...
- [x] 12.20 Desktop steps 1–2 popover still painted stale then jumped: notification prefs fetch swapped the 96px loading placeholder for the full toggle list AFTER highlight — the content-sized, vertically centered desktop dialog grew and re-centered, moving both profile targets (mobile bottom sheet is anchored, steps 3–8 sheets have no late height growth). Fix in `map-page.vue` `stageTourSurface('profile')`: kick `notificationsStore.loadPrefs()` before the waypoint spotlights and await it before returning, so the sheet mounts with prefs cached and the layout is final before the stillness gate
