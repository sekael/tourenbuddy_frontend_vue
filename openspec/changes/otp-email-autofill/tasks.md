...
## 6. Brevo template audit (manual, out of repo — the only task that affects real autofill)

- [ ] 6.1 In the Brevo dashboard, open the EN (`BREVO_TEMPLATE_EN`) and DE (`BREVO_TEMPLATE_DE`) OTP templates. Check subject and body against the heuristic in 2.3; adjust wording if needed. Record the before/after text in `docs/otp-and-email-autofill.md` — this edit has no git trail and no rollback (design "Risks")
