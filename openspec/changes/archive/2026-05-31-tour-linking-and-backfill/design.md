...
### D17. Pending-request invalidation: server trigger + client warning, parallel to eviction

The current eviction trigger only fires for tours already in a `tour_link_group`. A tour that is the initiator (or target) of a `pending` `tour_link_request` but not yet grouped is invisible to the existing eviction/void-pending pipeline. If the user edits such a tour so the collision predicate no longer holds (goal moves >200 m from the counterpart tour's goal, `tour_type` diverges, visibility flips away from `friends`), the row stays `pending`; the target then sees `predicate_failed` on accept. Two coupled fixes:

- **DB:** Extend `fn_void_pending_requests_on_invariant_break` (or a sibling trigger function) to fire on AFTER UPDATE on `tours` whenever a column relevant to the predicate changes (`goal`, `tour_type`, `visibility`). The function re-evaluates `fn_collision_predicate(initiator_tour_id, target_tour_id)` for every `pending` request where the updated tour is initiator or target; rows where the predicate is now false move to `withdrawn` with `resolved_at = now()`.
- **Client:** The `wouldEvict` gate in `tour-info-sheet.vue` is generalized to `wouldBreakLink`, which returns true when either (a) the tour is grouped and the existing eviction conditions hold, OR (b) the tour has at least one outgoing/incoming `pending` request and the proposed change would cause `fn_collision_predicate` to return false for that request. The same `link-edit-warning-dialog.vue` is reused; copy adapts to "this will cancel your pending link request" when no group exists.

**Why a trigger and not only client gating:** the client gate is best-effort — direct REST writes from another device, race conditions, or bugs can still produce inconsistent state. The trigger guarantees the invariant. The client gate is purely UX so the user is not surprised. **Why the same dialog:** users do not distinguish between "linked" and "linking-in-progress" mentally; one dialog with adaptive copy is simpler than two.

**Notification policy:** auto-withdrawal inherits the manual-withdrawal policy. Per Task 3.3 `withdraw` does not currently dispatch. The client SHALL NOT call `notifyTourLinkRequestEvent(requestId, 'withdrawn')` for trigger-driven auto-withdrawals (would be redundant and dispatch happens, if at all, via the same code path). If the manual-withdraw policy is later changed to notify recipients, auto-withdrawals SHALL be included automatically — no second policy.

### D18. In-app backfill entry point + My Tours tab persistence

The friendship-accept digest and the friend-profile collisions row (D14) both scope backfill to a single friendship. The user also needs a discovery surface that aggregates **all** of their reconcilable collisions in one place, independent of digest delivery. Decisions:

- **Entry point on Friends tab:** The My Tours list (`tour-list-sheet.vue`) gains a single button on the Friends tab — "View backfill collisions" — visible whenever the user has at least one friend, regardless of `tour_interest` mute preference. Tapping opens the backfill page in the same surface (bottom sheet on mobile, side drawer on desktop) the My Tours list uses, NOT as a separate route. **Why same surface:** routing to `/friends/.../backfill-collisions` opens a full-page route that breaks the My Tours sheet context and requires a separate back stack. Reusing the sheet/drawer keeps the navigation feel "drill-down → back" and avoids a full-page transition on mobile. The existing digest deeplink route (`/friends/:friendshipId/backfill-collisions`) is retained for notification deeplinks (cold-start safe) and continues to scope to a single friendship.
- **Backfill scope = all friendships:** `listBackfillCollisionsForFriendship(friendshipId)` gains a sibling repository method `listAllBackfillCollisions()` that scans across every accepted friendship of the viewer. The page distinguishes the two modes by route / prop. The all-friendships mode includes a per-row friend-name label since pairs span multiple friendships.
- **Back navigation:** The in-sheet backfill view's back button closes the backfill view and restores the My Tours list. Because the entry button lives on Friends, the restored list is automatically on the Friends tab — no extra logic needed beyond a local view-state toggle inside the sheet.
- **Persist active tab across sessions:** `useActiveTourTab` currently holds the selected tab in a module-level `ref`. It SHALL persist to `localStorage` (`tours.list.activeTab`) so the tab survives full reloads and PWA restarts. The persistence layer SHALL ignore unknown values (forward-compat) and fall back to `owned`.

**Why a button rather than a third tab:** a "Backfill" tab implies a third primary list of tours equivalent to Owned/Friends; the data is actually a list of **pairs** (your tour ⇄ friend's tour), so it would look out-of-place under a tour-list tab. A button-as-action accurately frames the surface as a derived view, not a parallel tour list. **Why on Friends specifically:** the action is conceptually friendship-derived; surfacing it under Owned would invite confusion ("are these my tours?"). On Friends it reads as "things you and your friends could collaborate on."
...
2. **Worker** updated together with the migration: `handleTourInterest` rewritten as a collision scanner; `handleFriendRequestResponded` extended with an independent `dispatchBackfillDigest` sub-routine (per D6 — no new endpoint); new lifecycle endpoints for link requests. Worker deploy is independent of frontend deploy (existing pattern via `wrangler deploy`).
