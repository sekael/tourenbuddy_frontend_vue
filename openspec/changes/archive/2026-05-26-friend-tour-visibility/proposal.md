...
- **Detail gating:** The friend-read view never exposes the owner's raw `partner_ids`. A friend who is *not* a partner on a tour additionally sees the partner names, `planned_date`, and `gpx_filepath` withheld. Gating is derived live in the read view via a `security definer` helper that resolves tour partners → registered users (no materialized table).
...
- **Friend tour browsing UI:**
  - **Map** shows owned tours + **all** readable friend tours (partner and non-partner), marked with a friendship icon in the marker and clustered like owned markers; gated detail still applies when a non-partner tour is opened.
  - **List** ("My Tours") defaults to owned tours, with separate **Owned** / **Friends** tabs — no merged list, independent search + filter per tab, and the active tab persists across detail navigation. Non-partner friend tours show in gated form.
...
- **Database (migrations):** new `tours.visibility` column + check constraint; a `tours` friend SELECT RLS policy (no `tour_partners` policy — partners resolve via the helper, raw `contact_id`s never leave the DB); `tour_partner_user_ids()` + tour-scoped `tour_partner_names()` `security definer` helpers; a new `security_invoker` `friend_tours_view` that gates partner names, `planned_date`, `gpx_filepath` for non-partner friend viewers; partner-friend `SELECT` policies on the `tour-gpx` + `tour-attachments` storage buckets and the `tour_attachments` table.
- **Frontend (`features/tours`):** tour schema + entity gain `visibility`/`isFriendTour`/`isPartner`/`partnerNames`; tours repository gains friend-tour reads; tours store gains a friends-tours collection; tour form + info sheet gain the visibility toggle; tour list sheet gains Owned/Friends tabs; map marker layer gains friend styling.
