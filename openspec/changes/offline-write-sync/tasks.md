...
- [x] 6.1 The `onSubscribed` transition kickstarts `replayQueue()` (task 5.2) AND gates the refetch: each in-scope store's `onSubscribed` refetch `await`s `replayDone` before assigning, so the drain completes before the overwrite — one event, closing the offline-app-cache-sync D6 clobber hazard. Shared seam `core/offline/reconnect.ts` `flushThenRefetch(refetch)` (single-flight drain, then refetch; failed flush still refetches). Wired in tours-store; contacts/profile/availability/friendships wire it as they land (7.2–7.5)
  - DONE: `flushThenRefetch(refetch)` now wired in ALL in-scope (queued) stores' `onSubscribed` (tours, contacts, profile, availability). Friendships stay online-only (not queued) → plain `fetchAll()` on reconnect, no flush gate. Ordering covered by `reconnect.test.ts` (10.4).
...
- [x] 7.5 Friendships — **online-only, NOT queued** (scope narrowed post-implementation). ALL friendship actions stay online-only (block-form `mutate`): `sendRequest` / `accept` / `deny` / `cancel` / `removeFriendship`. RATIONALE: friendship responses depend on too many connection-bound RPCs (requester identity/name lookups to render and act on a request, `findUsersByPhones` phone-registration to resolve a send target) — queueing them offline is not reliably actionable, so the whole surface is simplified to online-only rather than partially queued. The earlier queue/replay wiring (`kind='friend-request'` / `kind='friendship'` handlers, composite-snapshot cache, deferred-notify seams) was REMOVED; the store is back to block-form (`accept`/`removeFriendship` drop offline via `mutate(fn)`, the rest are plain online calls). UI: friend-request action buttons (accept / deny / cancel / block in the requests sheet, send / accept in the connect prompt) are `:disabled="!isOnline"` with an `offlineSync.friendshipsOnlineOnly` hint. Friend TOURS remain read-only — no handler, no queue.
  - DONE: friendships-store reverted to online-only (block-form). No friendship replay handlers registered. `onSubscribed: () => fetchAll()` (plain reconnect refetch — friendships never enqueue, so no flush gate needed). UI gating added to `friend-requests-sheet.vue` + `connect-prompt.vue`; i18n key `offlineSync.friendshipsOnlineOnly` (en + de-CH).
- [x] 7.6 Conflict handling (LWW governs `op=update` only): replay compares server `updated_at` vs entry `baseUpdatedAt` (DC5(i)); server-newer → skip + surface. `op=delete` replays unconditionally. `op=update` on a server-deleted row → dead-letter, never resurrect (guaranteed by update-only `update_tour_full`). If any in-scope table lacks a reliable `updated_at`, add it via a local-first migration (DC5(ii)) — `supabase migration new <name>`, `supabase db reset` to verify; prompt before `db push`
  - DONE: LWW gate already lives in each `op=update` replay handler (tours/contacts/profile) — server-newer `updated_at` throws `PermanentReplayError('… LWW loser')`. SURFACE: `deadLetter(entityId, reason)` now records `deadReason`; replay tags an LWW loser `'conflict'` (regex on the error message) vs `'permanent'`, and the dead-letter review surface shows the `offlineSync.conflictLost` copy for conflicts. Availability is whole-set offline-wins (no `updated_at`, unconditional). Friendships are online-only (not queued) → no LWW. No new migration needed — every in-scope LWW table already got `updated_at` in `20260811085649`.
