...
- [ ] 1.1 Cut a fresh branch from latest main (after offline-app-cache-sync is merged + archived): `git fetch origin && git checkout main && git pull && git checkout -b feat/245-offline-write-sync`

## 2. Durable write queue (`src/core/offline/`)

- [ ] 2.1 `write-queue.ts` — add object store `write-queue` to the existing `tourenbuddy-data-cache` DB (bump `DB_VERSION`, create store in `onupgradeneeded`), keyed by `entityId` (ONE entry per entity). Entry `{ entityId, kind, op, payload, blobs?, baseSnapshot?, baseUpdatedAt?, seq, attempts }` where `op ∈ {create,update,delete}`, blobs as native `Blob`. Ops: `getEntry(entityId)`, `peekAllOrdered()` (by `seq`), `remove(entityId)`, `bumpAttempt(entityId)`, `deadLetter(entityId)` + a `dead-letter` store
- [ ] 2.2 `coalesce(existing, incoming)` — pure fn implementing the DC1 table: create+update→`op=create` (merged payload); create+delete→ANNIHILATE (return null); update+update→`op=update` keeping FIRST `baseSnapshot`/`baseUpdatedAt`; update+delete→`op=delete`; delete terminal. **This is the core logic gap — write it test-first**
- [ ] 2.3 Enqueue quota guard: before enqueue, `navigator.storage.estimate()` headroom check (same pattern as offline-map-support/offline-app-cache-sync); if short, refuse with a clear message rather than corrupt the queue
...
- [ ] 4.1 `kind → handler` registry (`registerReplay(kind, handler)`), populated by stores at init. Handler replays by `entry.op`: `create` → `create_tour_full` (ON CONFLICT DO NOTHING) + best-effort GPX; `update` → `update_tour_full` (update-only; 0-rows/404 → dead-letter, never resurrect); `delete` → delete; then deferred notify keyed on `entry.op`
- [ ] 4.2 `replayQueue()` — drain `peekAllOrdered()` (by `seq`, cross-entity FK order): dispatch each entry; success → `remove(entityId)`; transient failure → `bumpAttempt` + capped backoff; permanent failure (4xx/404/LWW-loser/over-cap) → `deadLetter` and continue (no head-of-line block)
- [ ] 4.3 Cross-entity dependency cascade: when a create for entity A dead-letters, dead-letter any pending entry whose `payload` references A (e.g. a tour whose `partnerIds` include a contact created offline that failed)
- [ ] 4.4 Expose a `replayDone` gate (promise resolving when a drain completes) for the reconnect-ordering wiring (task 6)

## 5. Reachability + flush trigger (`src/core/offline/use-reachability.ts`)

- [ ] 5.1 Reachability PREDICATE (is-backend-reachable), cheapest-first: `navigator.onLine===false`→no (true is NOT proof) → realtime WS `status===SUBSCRIBED` (proven) → flush-attempt outcomes. Last-resort tier: a single capped/backed-off health `HEAD` only when a write is pending and cheaper tiers are inconclusive. NO fixed polling loop (`// ponytail:` the ceiling + upgrade path). Keep this SEPARATE from the flush triggers (5.2)
- [ ] 5.2 Flush TRIGGERS (kickstart): authoritative = the realtime WS `SUBSCRIBED`/`onSubscribed` transition (proven reachability, the SAME event DC4/task 6.1 gates the refetch on). Plus nudges: app foreground (`visibilitychange`→visible), a new enqueue while already reachable, and the DC9 backed-off retry. Do NOT use the coarse `online` window event as the authoritative kickstart (false positives)
