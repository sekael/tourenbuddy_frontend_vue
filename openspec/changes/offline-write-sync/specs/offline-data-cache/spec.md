...
tours, contacts, profile, availability, and friendship actions) durably, apply
them optimistically to local state and the read cache, and replay them
automatically when connectivity returns. It SHALL NOT silently drop an offline
mutation. Friend tours remain read-only and SHALL NOT be queued. Mutations made
while online SHALL behave exactly as before (no queue involvement).
