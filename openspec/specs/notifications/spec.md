...
The system SHALL hide the push toggle and show install instructions when the client is iOS Safari and the app is not running in standalone (installed PWA) mode.

#### Scenario: iOS browser, not installed
- **WHEN** the user opens preferences in iOS Safari and `display-mode` is not `standalone`
- **THEN** the push toggle is hidden and an install-to-home-screen guidance block is shown
