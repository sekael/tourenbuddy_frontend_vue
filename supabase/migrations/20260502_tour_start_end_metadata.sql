-- Migration: Add start/end point name and elevation metadata to tours (Issue #92)
-- All new columns are nullable for backward compatibility with existing tours.

-- 1. Add new columns to tours table
alter table public.tours
  add column if not exists start_point_name text null,
  add column if not exists start_point_elevation integer null,
  add column if not exists end_point_name text null,
  add column if not exists end_point_elevation integer null;

-- 2. Replace tours_view to include the four new columns.
...
-- 3. Update create_tour_full to accept the four new parameters.
...
-- 4. Update update_tour_full to accept the four new parameters.
